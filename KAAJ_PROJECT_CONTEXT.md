...
---
## 14. Product Page & Sizing Refinements (August 19-20, 2026)
*   **Streamlined UI:** Removed the redundant "Find my size" CTA from the variant selector block to maintain the minimalist aesthetic.
*   **Native Share Functionality:** Implemented a native Web Share API button (with a clipboard fallback) directly next to the product price on the PDP. Designed using the universal "three-node network" share icon.
*   **Dynamic, Product-Specific Sizing Engine:** Re-architected `constants.ts` to dynamically calculate exact product dimensions based on the specific `product.name`.
    *   **Baseline + Overrides:** The engine uses standard baseline dimensions (Shoulder, Chest, Sleeve Length, Waist, Hip, Thigh) and merges them with product-specific manual overrides (Length, Sleeve Opening, Bottom Hem) sourced directly from handwritten specifications.
    *   **Mapping Rules:** Exact naming mappings were applied for edge cases (e.g., "Rose Dust" utilizes the Angarkha specifications, while "Blush Heaven" utilizes the standard Rose Dust specs).
*   **Size Guide Simplicity:** 
    *   **Modal:** Stripped out all complex data tables from `SizeGuideModal.tsx`, leaving only the visual measurement diagrams and essential notes to reinforce the "Quiet Luxury" visual hierarchy.
    *   **Dupatta Removal:** Completely removed the "Dupatta" tab and sizing chart from both the popup modal and the dedicated `/size-guide` page.
